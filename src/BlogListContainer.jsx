...
// use a local array in place of an API data call
const data = [{
  id: '1akf',
...
class BlogListContainer extends Component {
  ...
  componentDidMount(){
    // simulate an API call .then update state, simulate below
    setTimeout(() => {
      this.setState({
        items: data,
  ...
  _onSearchQuery(query){
    if (!query || query.length === 0) {
      this.setState({
        items: data // reset the items if the query length is 0
      });
      return;
    }
    ...
    const list = (
      <div className='rp-bloglist-container'>
        <BlogList
          ...
          onQueryChange={(val) => this._onSearchQuery(val)}
        />
      </div>
    );
